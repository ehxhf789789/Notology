...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    /// Read a file: GET. Uses 5-minute timeout for large files.
    ...
    /// Write a file: PUT with optional conditional If-Match header.
    /// Timeout scales with file size: 60s base + 60s per 50MB.
    /// Returns the ETag from the response headers if available (avoids extra PROPFIND).
    /// If `expected_etag` is provided, sends If-Match header — server returns 412 if etag mismatch.
    pub async fn put_file(&self, path: &str, content: &[u8]) -> Result<Option<String>, String> {
        self.put_file_conditional(path, content, None).await
    }

    /// PUT with optional If-Match conditional header.
    pub async fn put_file_conditional(&self, path: &str, content: &[u8], expected_etag: Option<&str>) -> Result<Option<String>, String> {
        let size_mb = content.len() as u64 / (1024 * 1024);
        let timeout_secs = 60 + (size_mb / 50) * 60;
        let mut headers = self.headers();
        // If-Match conditional PUT — prevents overwriting concurrent changes.
        // Supports both strong ("etag") and weak (W/"etag") ETags.
        if let Some(etag) = expected_etag {
            // If the value already contains quotes, send as-is; otherwise wrap.
            let header_val = if etag.contains('"') {
                etag.to_string()
            } else {
                format!("\"{}\"", etag)
            };
            if let Ok(val) = HeaderValue::from_str(&header_val) {
                headers.insert("If-Match", val);
            }
        }
        let resp = self.client
            .put(&self.url(path))
        ...
        match resp.status() {
            s if s.is_success() => {
                // Extract ETag from PUT response to avoid extra PROPFIND round-trip
            ...
            // 1-3: 412 Precondition Failed = another device modified the file
            StatusCode::PRECONDITION_FAILED => Err("precondition_failed".to_string()),
    ...
    /// 1-2: Atomic upload — PUT to .tmp file, then MOVE to final path.
    /// Prevents half-written files on NAS if network drops during upload.
    pub async fn put_file_atomic(&self, path: &str, content: &[u8], expected_etag: Option<&str>) -> Result<Option<String>, String> {
        let tmp_path = format!("{}.notology-tmp", path);
        // Upload to temp file (no If-Match on temp — it's always new)
        self.put_file_conditional(&tmp_path, content, None).await?;

        // MOVE temp → final (atomic on most filesystems)
        let mut headers = self.headers();
        let dest_url = self.url(path);
        ...
        // If-Match on the destination (original file) to prevent overwriting concurrent changes
        if let Some(etag) = expected_etag {
        ...
        let resp = self.client
            .request(Method::from_bytes(b"MOVE").unwrap(), &self.url(&tmp_path))
            .headers(headers)
            .send()
            .await
            .map_err(|e| {
                // Cleanup: delete orphaned tmp file
                log::warn!("[webdav] MOVE failed, orphaned tmp file: {}", tmp_path);
                format!("MOVE failed: {}", e)
            })?;

        match resp.status() {
            s if s.is_success() => {
                // Get ETag of final file after MOVE
                Ok(self.get_metadata(path).await.ok().and_then(|m| m.etag))
            }
            StatusCode::PRECONDITION_FAILED => {
                // Concurrent modification detected — cleanup tmp
                let _ = self.delete_file(&tmp_path).await;
                Err("precondition_failed".to_string())
            }
            status => {
                let _ = self.delete_file(&tmp_path).await;
                Err(format!("MOVE returned {}", status))
            }
        }
    }
